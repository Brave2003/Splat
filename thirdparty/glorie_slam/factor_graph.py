...
class FactorGraph:
    ...
    def rm_keyframe(self, ix):
        ...
        with self.video.get_lock():
            ...
            self.video.depths[ix] = self.video.depths[ix + 1]
            self.video.mask_ori[ix] = self.video.mask_ori[ix + 1]
    ...
    @torch.cuda.amp.autocast(enabled=True)
    @torch.no_grad()
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False, opt_type=None,use_mono=False):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            self.video.ba(target, weight, damping, ii, jj, t0, t1,
                          iters=itrs, lm=1e-4, ep=0.1, motion_only=motion_only, opt_type=opt_type,use_mono=use_mono)
