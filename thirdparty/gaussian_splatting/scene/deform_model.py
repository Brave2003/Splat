...
from thirdparty.gaussian_splatting.utils.general_utils import get_expon_lr_func
import numpy as np
from thirdparty.gaussian_splatting.utils.general_utils import helper
...
class DeformModel:
    ...
    def train_setting(self, training_args):
        ...
        l = [
            {
                'params': group['params'],  # 参数张量
                'lr': training_args.position_lr_init * self.spatial_lr_scale * training_args.deform_lr_scale,
                # 初始学习率 = 基础学习率 * 空间缩放系数 * 形变缩放系数
                "name": group['name']  # 参数组名称（如"位移场"、"旋转场"）
            }
            for group in self.deform.trainable_parameters()  # 遍历形变模型的所有可训练参数组
        ]

        # 初始化Adam优化器（学习率占位为0，实际由参数组定义）
        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)

        # 配置形变场学习率的指数衰减调度器参数
        self.deform_scheduler_args = get_expon_lr_func(
            lr_init=training_args.position_lr_init * self.spatial_lr_scale * training_args.deform_lr_scale,  # 初始学习率
            lr_final=training_args.position_lr_final * training_args.deform_lr_scale,  # 最终学习率
            lr_delay_mult=training_args.position_lr_delay_mult,  # 学习率延迟倍数（控制衰减速度）
            max_steps=training_args.deform_lr_max_steps  # 最大训练步数
        )

        # 记录关键参数到类属性
        self.lr_init = training_args.position_lr_init * self.spatial_lr_scale * training_args.deform_lr_scale  # 初始学习率
        self.lr_final = training_args.position_lr_final * training_args.deform_lr_scale  # 最终学习率
        self.lr_delay_mult = training_args.position_lr_delay_mult  # 学习率衰减延迟系数
        self.max_steps = training_args.deform_lr_max_steps  # 最大优化步数
        #if self.name == 'node':
