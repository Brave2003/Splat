...
class Embedder:
    ...
    def create_embedding_fn(self):
        ...
        for freq in freq_bands:
            for p_fn in self.kwargs['periodic_fns']:
                ...
                embed_fns.append(partial(self.periodic_fn, p_fn, freq))
                out_dim += d
    ...
    def periodic_fn(self, p_fn, freq, x):
        return p_fn(x * freq)
        
    @classmethod
    def create_periodic_fn(cls, p_fn, freq):
        def periodic_fn(x):
            return p_fn(x * freq)
        return periodic_fn
...
class DeformNetwork(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, t_multires=6, multires=10,
        ...
        self.local_frame = local_frame  # False
        if self.local_frame:
        ...
        for layer in self.linear:  # self.linear ������λ�ú�ʱ�������
        ...
        nn.init.zeros_(self.gaussian_warp.bias)  # ���dxyz
...
def farthest_point_sample(xyz, npoint):
    ...
    """
    # ��Զ���²���,ÿ�β���һ�����ʹ�þ��뵱ǰ����Զ�ĵĵ���Ϊ��һ�������㡣
    device = xyz.device
    B, N, C = xyz.shape  # 1, N, 3
    centroids = torch.zeros(B, npoint, dtype=torch.long).to(device)
    distance = torch.ones(B, N).to(device) * 1e10
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to(device)  # �����ѡһ������Ϊ��Զ��
    batch_indices = torch.arange(B, dtype=torch.long).to(device)
    for i in range(npoint):
        centroids[:, i] = farthest
        centroid = xyz[batch_indices, farthest, :].view(B, 1, C)  # ��ѡ��λ��
        dist = torch.sum((xyz - centroid) ** 2, -1)  # ��ѡ�������е�ľ���
        mask = dist < distance
        distance[mask] = dist[mask]
        farthest = torch.max(distance, -1)[1]  # ����Զ����Ϊ��һ����ѡ��
    return centroids
...
class ControlNodeWarp(nn.Module):
    def __init__(self, is_blender, init_pcl=None, node_num=512, K=5, use_hash=False, hash_time=False, enable_densify_prune=False, pred_opacity=False, pred_color=False, with_arap_loss=False, with_node_weight=True, local_frame=False, d_rot_as_res=True, skinning=False, hyper_dim=2, progressive_brand_time=False, max_d_scale=-1, is_scene_static=False, **kwargs):
        super().__init__()
        self.K = K
        self.use_hash = use_hash  # False
        self.hash_time = hash_time  # False
        self.enable_dp = enable_densify_prune  # False ??
        self.name = 'node'
        ...
        self.skinning = skinning  # As skin model, discarding KNN weighting  ## False
        ...
        self.register_buffer('inited', torch.tensor(False))  # self.inited = False
        self.nodes = nn.Parameter(torch.randn(node_num, 3+self.hyper_dim))  # [node_num, 3(xyz��λ��) + 8(��+��ת��Ԫ��+ƽ������)]
        if not self.skinning:
            self._node_radius = nn.Parameter(torch.randn(node_num))  #_node_radius���ã�densify�ͼ���Ȩ��cal_nn_weightʱʹ�õ�
    ...
    def init(self, opt, init_pcl, hyper_pcl=None, keep_all=False, force_init=False, as_gs_force_with_motion_mask=False, force_gs_keep_all=False, reset_bbox=True, **kwargs):
        ...
        self.register_buffer('inited', torch.tensor(True))  # self.inited = True
        if keep_all or self.node_num > init_pcl.shape[0]:  # ���ÿ��Ƶ�����������������������:
            self.nodes = nn.Parameter(torch.cat([init_pcl.float(), 1e-2 * torch.ones([init_pcl.shape[0], self.hyper_dim]).float().cuda()], dim=-1))  # ʹ�����и�˹��λ�ó�ʼ�����Ƶ�
            init_nodes_idx = None
            ...
            self.nodes.data = nn.Parameter(torch.cat([init_pcl[init_nodes_idx].float(), 1e-2 * torch.ones([self.node_num, self.hyper_dim]).float().cuda()], dim=-1))  # [node_num, 3(xyz��λ��) + 8(�뾶o+��ת��Ԫ��+ƽ������)] ��3(xyz��λ��)��init_pcl��ֵ
        ...
        if self.skinning:
            ...
            if keep_all or self.node_num > init_pcl.shape[0]:
                self._node_radius = nn.Parameter(torch.log(.1 * scene_range + 1e-7) * torch.ones([self.node_num]).float().to(scene_range.device))  # ��ʼ�����Ƶ�İ뾶
    ...
    def extend_node(self, init_pcl, keep_all=False, force_init=False, reset_bbox=True, sample_number=250, **kwargs):
        if keep_all or self.node_num > init_pcl.shape[0]:  # ���ÿ��Ƶ�����������������������:
            new_nodes = nn.Parameter(torch.cat([init_pcl.float(), 1e-2 * torch.ones([init_pcl.shape[0], self.hyper_dim]).float().cuda()], dim=-1))  # ʹ�����и�˹��λ�ó�ʼ�����Ƶ�
            init_nodes_idx = None
            print('add new contorl node.')
        else:
            pcl_to_samp = init_pcl
            init_nodes_idx = farthest_point_sample(pcl_to_samp.detach()[None], sample_number)[0]
            new_nodes = nn.Parameter(torch.cat([init_pcl[init_nodes_idx].float(), 1e-2 * torch.ones([sample_number, self.hyper_dim]).float().cuda()], dim=-1))  # [node_num, 3(xyz��λ��) + 8(�뾶o+��ת��Ԫ��+ƽ������)] ��3(xyz��λ��)��init_pcl��ֵ
        scene_range = init_pcl.max() - init_pcl.min()
        if self.skinning:
            if 'feature' in kwargs:
                gs_weights = kwargs['feature']
                radius = .1 * scene_range + 1e-7
                initial_weights = - torch.log((init_pcl[:, None] - new_nodes[None,...,:3]).square().sum(dim=-1) / radius ** 2)
                gs_weights.data = initial_weights
        else:
            if keep_all or self.node_num > init_pcl.shape[0]:
                new_node_radius = nn.Parameter(torch.log(.1 * scene_range + 1e-7) * torch.ones([new_nodes.shape[0]]).float().to(scene_range.device))  # ��ʼ�����Ƶ�İ뾶
                new_node_weight = nn.Parameter(torch.zeros_like(torch.zeros_like(new_nodes[:, :1])))
            else:
                new_node_radius.data = nn.Parameter(torch.log(.1 * scene_range + 1e-7) * torch.ones([new_nodes.shape[0]]).float().to(scene_range.device))
                new_node_weight.data = torch.zeros_like(torch.zeros_like(new_nodes[:, :1]))
        if self.with_node_weight:
            return [new_nodes, new_node_radius, new_node_weight]
        else:
            return [new_nodes, new_node_radius]
    ...
    def cal_nn_weight(self, x:torch.Tensor, K=None, feature=None, nodes=None, gs_kernel=True, temperature=1.):
        ...
        if self.skinning:
            ...
            if self.cached_nn_weight and self.nn_weight is not None:
                ...
                if gs_kernel:
                    nn_radius = self.node_radius[nn_idxs]  # N, K  # ��������self._node_radius
    ...
    def query_network(self, x, t, **kwargs):
        values = self.network(x=x, t=t, **kwargs)  # 410�е�forward()
        return values
    ...
    def arap_loss(self, t=None, delta_t=0.05, t_samp_num=2, cur_time=None):
        t = torch.rand([]).cuda() if t is None else t.squeeze() + delta_t * (torch.rand([]).cuda() - .5)
        if cur_time is not None:
            t = torch.clamp(t, 0+0.5*delta_t, cur_time-0.5*delta_t)
        t_samp = torch.rand(t_samp_num).cuda() * delta_t + t - .5 * delta_t
        #if cur_time is not None:
        #    t_samp = torch.clamp(t_samp, 0, cur_time)
        t_samp = t_samp[None, :, None].expand(self.node_num, t_samp_num, 1)  # M, T, 1
    ...
    def elastic_loss(self, t=None, delta_t=0.005, K=2, t_samp_num=8, cur_time=None):  # APAR Loss in paper
        # Calculate nodes translate
        t = torch.rand([]).cuda() if t is None else t.squeeze() + delta_t * (torch.rand([]).cuda() - .5)
        if cur_time is not None:
            t = torch.clamp(t, 0+0.5*delta_t, cur_time-0.5*delta_t)
        # ����t_samp_num������ʱ��㣬��Χ��[t - 0.5 * delta_t, t + 0.5 * delta_t]֮��
        t_samp = torch.rand(t_samp_num).cuda() * delta_t + t - .5 * delta_t  # ����[-delta_t + t, delta_t + t]֮���8����
        #if cur_time is not None:
        #    t_samp = torch.clamp(t_samp, 0, cur_time)
    ...
    def acc_loss(self, t=None, delta_t=.005, cur_time=None):
        # Calculate nodes translate
        t = torch.rand([]).cuda() if t is None else t.squeeze() + delta_t * (torch.rand([]).cuda() - .5)
        if cur_time is not None:
            t = torch.clamp(t, 0+delta_t, cur_time-delta_t)
    ...
    def forward(self, x, t, feature, motion_mask, iteration=0, is_training=True, node_trans_bias=None, node_scaling_bias=None, animation_d_values=None, **kwargs):
        # ͨ��MLPԤ����Ƶ��dxyz, dscale, drot, ʹ��cal_nn_weight[N, 3]������Ƶ��3�������˹��Ȩ��
        if t.dim() == 0:
        ...
        # Animation
        if animation_d_values is not None:  # animation_d_vaules = None
        ...
        if self.local_frame:  # False
            ...
            translate = (node_trans[nn_idx] * nn_weight[..., None]).sum(dim=1)  # [N, 3, 3]*[N, 3, 1] -> [N, 3, 3] -> sum(dim=1) -> [N, 3]
        translate = translate * motion_mask

        ##############################################################################
        if not self.d_rot_as_res:  # False
            ...
            if node_trans_bias is not None:  # None
                with torch.no_grad():
            ...
            if node_trans_bias is not None:  # is None
                with torch.no_grad():
                    cur_node = self.nodes[..., :3] + node_trans
                    cur_gs = x + translate
    ...
    def as_gaussians(self):
        if not hasattr(self, 'gs') or self.gs is None:
            ...
            from gaussian_splatting.scene.gaussian_model import GaussianModel, BasicPointCloud, StandardGaussianModel
    ...
    def as_gaussians_visualization(self):
        from gaussian_splatting.scene.gaussian_model import GaussianModel, BasicPointCloud
    ...
    def cal_node_importance(self, x:torch.Tensor, K=None, weights=None, feature=None):
        ...
        if feature is not None:
            ...
            avg_affected_x = torch.zeros_like(self.nodes[:, :3])
        ...
        #print("avg_affected_x shape:", avg_affected_x.shape)
        #print(((nn_weight * weights[:, None]).view(-1, 1) * x[:, None].expand(*nn_weight.shape, x.shape[-1]).reshape(-1, x.shape[-1])).shape)
    ...
    def densify(self, max_grad, optimizer, x:torch.Tensor, x_grad: torch.Tensor, feature=None, K=None, use_gaussians_grad=False, force_dp=False):
        ...
        print("mean: ",node_edge_count.mean(), "max", node_edge_count.max())
        ...
        if pruned_pts_mask.sum() > 0:
            ...
            param_idx = np.arange(len(param_list))
            for group in optimizer.param_groups:
                if group["name"] != 'nodes':  #  {'params': [self.nodes, self._node_radius], 'name': 'nodes'} prune self.nodes��self._node_radius����
                    continue
                for i in param_idx:
                    stored_state = optimizer.state.get(group['params'][i], None)
                    if stored_state is not None:
            ...
            for key in optimizable_tensors:
                setattr(self, key, optimizable_tensors[key])  # ��prune������浽self.nodes��self._node_radius��
        else:
            pruned_pts_mask = ~pruned_pts_mask
        ...
        #self.gs._xyz.data = self.nodes[..., :3]  # self.node����ĸ�˹self.gs��ʲô����,ֻ��Ԥѵ������ʹ�õ�����
